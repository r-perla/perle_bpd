...
generated quantities {
  ...
  
  {
    ...
    for (r in 1:R_tilde) {
      ...
      log_lik[r] = normal_lpdf(est_tilde[r] | est_model[p, pr, f], sigma_tilde[p]);
      
      // Compute mean absolute error
      ae[r] = abs(est_tilde[r] - est_model[p, pr, f]);
      
      // Generate prediction
      y_rep[r] = normal_rng(est_model[p, pr, f], sigma_tilde[p]);
