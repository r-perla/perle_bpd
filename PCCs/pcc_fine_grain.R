...
# Load model objects
model_object <- read_rds("Replication/fitted_models/fine_grain_fit.rds")
...
ggsave(paste0(out_path, "fine_gran_expectation.svg"), width = 12, height = 8, dpi = 300)
...
ggsave(paste0(out_path, "fine_gran_mean.svg"), width = 6, height = 4, dpi = 300)
...
ggsave(paste0(out_path, "fine_gran_sd.svg"), width = 6, height = 4, dpi = 300)
...
ggsave(paste0(out_path, "fine_gran_skewness.svg"), width = 6, height = 4, dpi = 300)

# Extract alpha parameter
alphas <- colMeans(extract(model_object, pars = "alpha")$alpha)

# Plot density of alpha
data.frame(alpha = alphas) %>%
  ggplot(aes(x = alpha)) +
  geom_density(color = "#1E2022", fill = "#00ADB5") +
