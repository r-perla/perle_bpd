...
looic <- matrix(NA, nrow = length(groups) * length(models), ncol = 3)
colnames(looic) <- c("group", "model", "looic")
...
for (i in seq_along(groups)) {
  ...
  for (j in seq_along(models)) {
    ...
    preds <- extract(current_model_fit, pars = "y_rep")$y_rep
    ...
    looic[(i - 1) * length(models) + j, ] <- c(current_group, current_model, looic_value)
  }
}
